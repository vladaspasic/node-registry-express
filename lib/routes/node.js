...
class Node {
	 ...
	 */
	get(key) {
		assert('Key must be a string', typeof key === 'string');
	...
	require(name) {
		...
		assert('File name must be a string', typeof name === 'string');
		...
		throw new Error(`No file with name '${name}' can be found in ${this.location}`);
	}
	 ...
	 */
	child(location, name) {
		assert('Can not create a child Node on a destroyed Node', !this._isDestroyed);
	 ...
	 */
	traverse() {
		assert('Can not traverse through a destroyed Node', !this._isDestroyed);
